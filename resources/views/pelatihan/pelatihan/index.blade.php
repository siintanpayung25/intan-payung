...
$(document).on('click', '.btn-detail', function() {
    ...
    // Menampilkan loading spinner sementara
    $('#modalDetailBody').html('<div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div>');

    // AJAX untuk mengambil data detail
    $.ajax({
        url: '{{ route("pelatihan.show", ":id") }}'.replace(':id', pelatihanId),
        method: 'GET',
        success: function(response) {
            if (response) {
                const formatDate = date => new Date(date).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' });
                const getValue = (obj, key, fallback = '-') => obj && obj[key] ? obj[key] : fallback;

                var detailHTML = `
                    <table class="table">
                        <tr><th>Nama</th><td>${getValue(response.pegawai, 'nama')}</td></tr>
                        <tr><th>Skala</th><td>${getValue(response.skala, 'nama')}</td></tr>
                        <tr><th>Bentuk</th><td>${getValue(response.bentuk, 'nama')}</td></tr>
                        <tr><th>Kategori</th><td>${getValue(response.kategori, 'nama')}</td></tr>
                        <tr><th>Jenis</th><td>${getValue(response.jenis, 'nama')}</td></tr>
                        <tr><th>Training Need Analysis (TNA)</th><td>${getValue(response.tna, 'nama')}</td></tr>
                        <tr><th>Nama Pelatihan</th><td>${response.nama}</td></tr>
                        <tr><th>Tanggal Mulai</th><td>${formatDate(response.tgl_mulai)}</td></tr>
                        <tr><th>Tanggal Selesai</th><td>${formatDate(response.tgl_selesai)}</td></tr>
                        <tr><th>Durasi</th><td>${response.durasi} jam pelajaran</td></tr>
                        <tr><th>Instansi</th><td>${getValue(response.instansi, 'nama', getValue(response.universitas, 'nama'))}</td></tr>
                        <tr><th>Jumlah Peserta</th><td>${getValue(response, 'jumlah_peserta')}</td></tr>
                        <tr><th>Rangking</th><td>${getValue(response, 'rangking')}</td></tr>
                        <tr><th>Link Bukti Dukung</th><td><a href="${getValue(response, 'link_bukti_dukung', '#')}" target="_blank">${getValue(response, 'link_bukti_dukung', '-')}</a></td></tr>
                        <tr><th>Nomor Sertifikat</th><td>${response.nomor_sertifikat}</td></tr>
                    </table>
                `;
                ...
                $('#modalDetailBody').html('Data detail pelatihan tidak ditemukan.');
            }
        },
        error: function(xhr, status, error) {
            console.error("Error AJAX:", error);
            $('#modalDetailBody').html('Gagal memuat detail pelatihan.');
        }
    });
});
