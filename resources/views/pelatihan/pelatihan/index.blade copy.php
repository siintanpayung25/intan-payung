    ...
    $(document).ready(function() {
        ...
        $('#hapus-terpilih').on('click', function() {
            ...
            if (selectedIds.length > 0) {
                ...
                // Tombol "Hapus" (menghapus data yang dipilih)
                $('#modalDeleteButton').on('click', function() {
                    $.ajax({
                        url: "{{ route('pelatihan.hapus_terpilih') }}", // Endpoint untuk menghapus data terpilih
                        type: "POST",
                        data: {
                            ids: selectedIds, // Kirim ID yang dipilih untuk dihapus
                        ...
                        success: function(response) {
                            ...
                            $('input[type="checkbox"]').prop('checked', false);
                            $('#hapus-terpilih').hide(); // Menyembunyikan tombol hapus setelah selesai
                        },
                        error: function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    <table class="table">
                        <tr>
                            <th>Nama</th>
                            <td>${response.pegawai ? response.pegawai.nama : '-'}</td>
                        </tr>
                        <tr>
                            <th>Skala</th>
                            <td>${response.skala ? response.skala.nama : '-'}</td>
                        </tr>
                        <tr>
                            <th>Bentuk</th>
                            <td>${response.bentuk ? response.bentuk.nama : '-'}</td>
                        </tr>
                        <tr>
                            <th>Kategori</th>
                            <td>${response.kategori ? response.kategori.nama : '-'}</td>
                        </tr>
                        <tr>
                            <th>Jenis</th>
                            <td>${response.jenis ? response.jenis.nama : '-'}</td>
                        </tr>
                        <tr>
                            <th>Training Need Analysis (TNA)</th>
                            <td>${response.tna ? response.tna.nama : '-'}</td>
                        </tr>
                        <tr>
                            <th>Nama Pelatihan</th>
                            <td>${response.nama}</td>
                        ...
                        <tr>
                            <th>Tanggal Mulai</th>
                            <td>${formatDate(response.tgl_mulai)}</td>
                        </tr>
                        <tr>
                            <th>Tanggal Selesai</th>
                            <td>${formatDate(response.tgl_selesai)}</td>
                        </tr>
                        <tr>
                            <th>Durasi</th>
                            <td>${response.durasi} jam pelajaran</td>
                        </tr>
                        <tr>
                            <th>Instansi</th>
                            <td>${response.instansi ? response.instansi.nama : '-'}</td>
                        </tr>
                        <tr>
                            <th>Jumlah Peserta</th>
                            <td>${response.jumlah_peserta ? response.jumlah_peserta : '-'}</td>
                        </tr>
                        <tr>
                            <th>Rangking</th>
                            <td>${response.rangking ? response.rangking : '-'}</td>
                        </tr>
                        <tr>
                            <th>Link Bukti Dukung</th>
                            <td><a href="${response.link_bukti_dukung ? response.link_bukti_dukung : '#'}" target="_blank">${response.link_bukti_dukung ? response.link_bukti_dukung : '-'}</a></td>
                        </tr>
                        <tr>
                            <th>Nomor sertifikat</th>
                            <td>${response.nomor_sertifikat}</td>
                        </tr>
                    </table>
                    `;
                        ...
                        var modal = new bootstrap.Modal(document.getElementById('modalDetail'), {
                            backdrop: 'static', // Mencegah penutupan modal dengan klik luar
