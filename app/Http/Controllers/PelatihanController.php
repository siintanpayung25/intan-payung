...
use App\Exports\Pelatihan\PelatihanEkspor;
use App\Exports\Pelatihan\PelatihanBackup;
use App\Imports\Pelatihan\PelatihanImpor; // 
use App\Exports\Pelatihan\PelatihanTemplateEksporCSV;
use App\Exports\Pelatihan\PelatihanTemplateEksporExcel;
...
class PelatihanController extends BaseController
{
    ...
    {
        if ($request->ajax()) {
            // ⚠️ PERBAIKAN 1: Hapus eager load 'instansi' yang bermasalah.
            $data = Pelatihan::with(['pegawai', 'kategori', 'jenis', 'tna', 'skala', 'bentuk'])
                ->select('pelatihans.*'); // Select semua kolom dari tabel utama
            ...
            // Pencarian untuk semua kolom (Logika Search)
            if ($request->get('search')['value']) {
                $search = $request->get('search')['value'];
                $data->where(function ($query) use ($search) {
                    // Pencarian Langsung di Tabel pelatihans
                    $query->where('pelatihans.nama', 'like', '%' . $search . '%')
                        ->orWhere('pelatihans.tgl_mulai', 'like', '%' . $search . '%')
                        ->orWhere('pelatihans.tgl_selesai', 'like', '%' . $search . '%')
                        ->orWhere('pelatihans.durasi', 'like', '%' . $search . '%')
                        ->orWhere('pelatihans.kategori_id', 'like', '%' . $search . '%')
                        ->orWhere('pelatihans.jenis_id', 'like', '%' . $search . '%')
                        ->orWhere('pelatihans.instansi_id', 'like', '%' . $search . '%') // Pencarian ID
                        ->orWhere('pelatihans.tna_id', 'like', '%' . $search . '%')
                        ->orWhere('pelatihans.nip', 'like', '%' . $search . '%')

                        // Pencarian Melalui Relasi Eloquent yang Aman (Nama Relasi di Model Pelatihan)
                        ->orWhereHas('pegawai', function ($q) use ($search) {
                            $q->where('pegawais.nama', 'like', '%' . $search . '%');
                        })
                        ->orWhereHas('kategori', function ($q) use ($search) {
                            $q->where('pelatihan_kategoris.nama', 'like', '%' . $search . '%');
                        })
                        ->orWhereHas('jenis', function ($q) use ($search) {
                            $q->where('pelatihan_jeniss.nama', 'like', '%' . $search . '%');
                        })
                        ->orWhereHas('tna', function ($q) use ($search) {
                            $q->where('pelatihan_tna.nama', 'like', '%' . $search . '%');
                        })
                        // ✅ PERBAIKAN 2: Logika Pencarian Nama Instansi/Universitas
                        ->orWhere(function ($subQuery) use ($search) {
                            $subQuery->whereHas('instansis', function ($q) use ($search) {
                                $q->where('instansis.nama', 'like', '%' . $search . '%');
                            })
                                // ATAU Mencari di Universitas (menggunakan relasi universitas)
                                ->orWhereHas('universitas', function ($q) use ($search) {
                                    $q->where('universitas.nama', 'like', '%' . $search . '%');
                                });
                        });
                });
            }

            // ⚠️ PERBAIKAN 3: Gunakan DataTables Builder Standar (Lebih kuat)
            return DataTables::of($data)
                ->addIndexColumn()

                // Kolom untuk menampilkan nama Instansi/Universitas (Memanggil Accessor)
                ->addColumn('instansi', function (Pelatihan $pelatihan) {
                    $detail = $pelatihan->detail_instansi; // Memanggil Accessor getDetailInstansiAttribute
                    return $detail ? $detail->nama : 'N/A';
                })

                // Kolom untuk menampilkan Nama Pegawai (jika tidak via eager load)
                ->addColumn('pegawai_nama', function (Pelatihan $pelatihan) {
                    return $pelatihan->pegawai->nama ?? 'N/A';
                })

                // Kolom untuk menampilkan Aksi
                ->addColumn('aksi', function (Pelatihan $item) {
                ...
                // Kolom Checkbox
                ->addColumn('checkbox', function (Pelatihan $item) {
                    return '<input type="checkbox" class="select-item" data-pelatihan_id="' . $item->pelatihan_id . '">';
                })

                // Pastikan kolom yang berasal dari HTML mentah diizinkan
    ...
    public function create()
    {
        ...
        $kategoris = Pelatihan_Kategori::all();  // Ambil semua kategori pelatihan

        // Ambil data instansi dari tabel instansis dan universitas
        $instansis = Instansi::all();  // Ambil semua instansi
        $universitas = Universitas::all();  // Ambil semua universitas

        // Gabungkan data instansi dan universitas
        $instansi_gabung_universitas = $instansis->merge($universitas);

        // Pastikan semua data dikirimkan ke view
        return view('pelatihan.pelatihan.create', compact(
            'pegawais',
            'skalas',
            'bentuks',
            'jeniss',
            'instansi_gabung_universitas',
            'tnas',
            'kategoris'
        ));
    }
        ...
        $request->validate([
            ...
            'instansi_id' => [
                ...
                function ($attribute, $value, $fail) {
                    // Memastikan ID ada di tabel instansis ATAU di universitas
                    $exists_in_instansi = DB::table('instansis')->where('instansi_id', $value)->exists();
                    $exists_in_universitas = DB::table('universitas')->where('instansi_id', $value)->exists();

                    if (!$exists_in_instansi && !$exists_in_universitas) {
                        $fail("Instansi yang dipilih tidak valid.");
                    }
                },
            ],
    ...
    public function edit($id)
    {
        ...
        $universitas = Universitas::all();  // Ambil data universitas
        ...
        // Pastikan semua data dikirimkan ke view
        return view('pelatihan.pelatihan.edit', compact(
            'pelatihan',
            'kategoris',
            'pegawais',
            'skalas',
    ...
    public function hapusTerpilih(Request $request)
    {
        ...
        // Cek jika ids null, bukan array, atau kosong
        if ($ids === null || !is_array($ids) || count($ids) === 0) {
            return response()->json(['message' => 'Pilih setidaknya satu data untuk dihapus.'], 400);
        }

        // Ambil data pelatihan yang dipilih
        $pelatihans = Pelatihan::whereIn('pelatihan_id', $ids)->get();

        // Loop untuk setiap pelatihan yang terpilih
        foreach ($pelatihans as $pelatihan) {
            // Cek apakah pelatihan memiliki tna_id
            if ($pelatihan->tna_id) {
                // Update status TNA di pelatihan_tna menjadi 0 hanya untuk tna_id yang sesuai
                $tna = Pelatihan_Tna::find($pelatihan->tna_id);
                if ($tna) {
                    $tna->update([
                        'status_tna' => 0  // Mengubah status menjadi 0
        ...
        // Penghapusan Pelatihan berdasarkan IDs
        $count = Pelatihan::whereIn('pelatihan_id', $ids)->delete();

        // Cek apakah ada data yang berhasil dihapus
        if ($count > 0) {
            return response()->json(['message' => "{$count} data Pelatihan berhasil dihapus."], 200);
        }
    ...
    public function show($id)
    {
        ...
        $pelatihan = Pelatihan::with('pegawai', 'skala', 'bentuk', 'kategori', 'jenis', 'tna', 'instansis', 'universitas')->findOrFail($id);
    ...
    public function impor(Request $request)
    {
        try {
            ...
            // Proses impor data
            Excel::import(new PelatihanImpor, $request->file('file'));
    ...
    public function templatePelatihanCSV()
    {
        // Menggunakan TnaTemplateExportCSV untuk mendownload template CSV
        return Excel::download(new PelatihanTemplateEksporCSV, 'template_tna.csv');
    }

    public function templatePelatihanExcel()
    {
        return Excel::download(new PelatihanTemplateEksporExcel, 'template_tna.xlsx');
    }
}
